...
def highlight_words_in_pdf(pdf_path, csv_file, status_message_var):
    try:
        ...
        for page_num in range(total_pages):
            ...
            highlight_page_with_synonyms(page, csv_file, synonym_counts, page_num + 1)
        ...
        if found_any_synonym:
            # 강조한 PDF 파일 저장
            highlighted_pdf_path = pdf_path[:-4] + HIGHLIGHTED_SUFFIX
            doc.save(highlighted_pdf_path)  # 파일 저장
            doc.close()  # 파일 닫기
            return found_any_synonym, synonym_counts, highlighted_pdf_path  # 강조한 PDF 파일 경로 반환
        else:
            doc.close()  # 파일 닫기
...
# 페이지에서 유의어 강조
def highlight_page_with_synonyms(page, csv_file, synonym_counts, page_num):
...
def create_result_message(found_any_synonym, synonym_counts, highlighted_pdf_path):
    if found_any_synonym:
        ...
        message = (
            "주의! 유의어가 발견되었습니다.\n\n"
            f"아래 경로에 유의어를 형광펜으로 강조한 파일이 저장되었습니다:\n"
            f"{highlighted_pdf_path}\n\n"
            f"발견된 유의어 목록:\n{synonym_report}\n"
        )

        txt_output_path = highlighted_pdf_path[:-4] + SYNONYM_COUNTS_SUFFIX
        save_synonym_counts_to_txt(synonym_counts, txt_output_path)

        message += (
            f"\n유의어 발견 횟수가 {txt_output_path}에 저장되었습니다."
        )
        return message, highlighted_pdf_path  # 메시지와 PDF 경로 반환
    else:
...
def process_files(pdf_file_path, csv_file_path, status_message_var):
    ...
    if len(result) == 4:  # 오류가 발생한 경우
        found_any_synonym, synonym_counts, highlighted_pdf_path, error_message = result
        status_message_var.set("오류")  # 상태 라벨에 오류 표시
        messagebox.showerror("오류 발생", error_message)  # 예외 메시지 알림창
        return

    found_any_synonym, synonym_counts, highlighted_pdf_path = result
    message, _ = create_result_message(found_any_synonym, synonym_counts, highlighted_pdf_path)
...
def setup_gui():
    ...
    # 라벨 및 버튼 가운데 정렬
    label = tk.Label(frame, text="PDF 파일에서 유의어를 형광펜으로 강조합니다.", padx=20, pady=5)
    label.pack()
